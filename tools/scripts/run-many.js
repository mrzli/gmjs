...
function runMany(argv) {
    ...
    var printAffectedCommand = "npx nx print-affected --base=".concat(base, " --head=").concat(head, " --target=").concat(target);
    console.log("printAffectedCommand: '".concat(printAffectedCommand, "'"));
    var affectedResult = (0, child_process_1.execSync)(printAffectedCommand).toString('utf-8');
    ...
    var projects = jobIndex < jobCount
    ...
    if (projects.length > 0) {
        (0, child_process_1.execSync)("npx nx run-many --target=".concat(target, " --projects=").concat(projects, " --parallel"), {
            stdio: [0, 1, 2]
        });
    }
}
function getAffectedProjects(affectedResult) {
    return JSON.parse(affectedResult)
        .tasks.map(function (t) { return t.target.project; })
        .slice()
        .sort();
}
